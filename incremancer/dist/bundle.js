...
(() => {
  "use strict";
  var distanceX = {};
  function distanceY(e, t) {
    return Math.sqrt(e * e + t * t);
  }

  function calculateDistanceAlternate(e, t, s, i) {
  ...
  function randomItem(e, t) {
    return e[Math.floor(t * e.length)];
  }

  function buildingDistance(e) {
    return numberOfParts(e, 2);
  }

  function unitHealth(e) {
    return numberOfParts(e, e > 1e3 ? 2 : 0);
  }

  function numberOfParts(e, t) {
    return (
      e || (e = 0),
      e >= 1e15
  ...
  function totalPrice(e, t, s, i) {
    return 1 == t
      ? e * i
      : e * ((Math.pow(t, s) * (Math.pow(t, i) - 1)) / (t - 1));
  }

  function adjustTooltipPosition(e, t) {
  ...
  function handleDragging(e) {
    if (zombieManager.zombieCursor) {
      zombieManager.zombieCursor.position = e.data.getLocalPosition(
        this.parent,
      );
      const t = e.data.getLocalPosition(tilingSprite);
      zombieManager.mouseOutOfBounds =
        t.x < 0 ||
        t.y < 0 ||
        t.x > tilingSprite.width ||
        t.y > tilingSprite.height;
    }
    if (e.data.originalEvent.touches && e.data.originalEvent.touches.length > 1)
      !(function (e) {
        ...
        B
          ? R + 50 < Date.now() &&
            Math.abs(t - B) > 10 &&
            (handleZoom(t > B ? 1 : -1, null), (R = Date.now()), (B = t))
          : (B = t);
      })(e);
      ...
      (this.x = e.x - this.dragOffset.x),
        ...
        confineToGameBounds(this),
        calculateDistanceAlternate(
          this.dragStartX,
          this.dragStartY,
          this.x,
          this.y,
  ...
  function confineToGameBounds(e) {
  ...
  function handleGroundClick(e) {
    this.hasMoved ||
      gameStateManager.currentState != gameStateManager.states.playingLevel ||
      (Y.shift
  ...
  function handleZoom(e, t) {
    if (R + 50 > Date.now()) return;
    R = Date.now();
    const s = sceneContainer;
    t ||
      (t = {
        x: 0.5 * D.x,
        y: 0.5 * D.y,
    ...
    e > 0
      ? s.scale.x < 10 &&
        ...
        zombieManager.zombieCursor &&
          zombieManager.zombieCursor.scale &&
          (zombieManager.zombieCursor.scale.x =
            zombieManager.zombieCursor.scale.y =
              1.1 * zombieManager.zombieCursor.scale.x))
      : Math.max(i, a) > 0.8 * Math.min(D.y, D.x) &&
        ...
        zombieManager.zombieCursor &&
          ...
          (zombieManager.zombieCursor.scale.x =
            zombieManager.zombieCursor.scale.y =
              0.9 * zombieManager.zombieCursor.scale.x)),
      (s.x = t.x - r * s.scale.x),
      (s.y = t.y - n * s.scale.y),
      confineToGameBounds(s);
  }
  ...
  function handleScrollWheel(e) {
    ...
    e.deltaY < 0 || e.deltaX < 0 ? handleZoom(1, t) : handleZoom(-1, t);
  }

  function Z(e = !1) {
    e &&
      ((sceneContainer.scale.x = D.defaultScale),
      (sceneContainer.scale.y = D.defaultScale),
      zombieManager.zombieCursor &&
        (zombieManager.zombieCursor.scale.x =
          zombieManager.zombieCursor.scale.y =
            zombieManager.zombieCursorScale * D.defaultScale)),
      (sceneContainer.x = (D.x - P.x * sceneContainer.scale.x) / 2),
      (sceneContainer.y = (D.y - P.y * sceneContainer.scale.y) / 2);
  }
  const G = {
      x: 0,
      y: 0,
      ...
      update() {
        (this.x = -sceneContainer.x / sceneContainer.scale.x),
          (this.y = -sceneContainer.y / sceneContainer.scale.y),
          (this.width = D.x / sceneContainer.scale.x),
          (this.height = D.y / sceneContainer.scale.y);
      },
    },
    X = new PIXI.Matrix();

  function gameLoop(e, t) {
    !(function (e) {
      const t = Y;
      let s = !1;
      const i = sceneContainer;
      t.w && ((i.y += t.scrollSpeed * e), (s = !0)),
        t.a && ((i.x += t.scrollSpeed * e), (s = !0)),
        t.s && ((i.y -= t.scrollSpeed * e), (s = !0)),
        t.d && ((i.x -= t.scrollSpeed * e), (s = !0)),
        s && confineToGameBounds(i);
    })(e),
      G.update(),
      (e *= gameStateManager.gameSpeed),
      graveyardManager.update(e),
      humanManager.update(e),
      zombieManager.update(e),
      golemManager.update(e),
      skeletonChampionManager.update(e),
      particleManager.update(e),
      (function (e, t) {
        if (
          (humanManager.vipEscaping && void 0 !== humanManager.vip
            ? (renderTextureSprite.alpha += e)
            : ((renderTextureSprite.alpha -= e),
              renderTextureSprite.alpha < 0 && (renderTextureSprite.alpha = 0)),
          renderTextureSprite.alpha > 0)
        ) {
          renderTextureSprite.alpha > 1 && (renderTextureSprite.alpha = 1),
            (renderTextureSprite.visible = !0),
            (renderTextureSprite.x = 5),
            (renderTextureSprite.y = D.y - 305);
          const e = sceneContainer.scale.x,
            s = sceneContainer.scale.y,
            i = sceneContainer.x,
            a = sceneContainer.y;
          sceneContainer.position.set(0, 0),
            humanManager.vip &&
              ((X.tx = -2 * humanManager.vip.x + 150),
              (X.ty = -2 * humanManager.vip.y + 150)),
            sceneContainer.scale.set(2, 2),
            t.renderer.render(sceneContainer, renderTexture, void 0, X),
            sceneContainer.scale.set(e, s),
            sceneContainer.position.set(i, a);
        } else renderTextureSprite.visible = !1;
      })(e, t);
  }

  function updateMap() {
    const e = Math.min(500 + 50 * gameStateManager.level, 1500),
      t = (Math.random() * e) / 3;
    (P = {
      x: e + t,
      y: e - t,
    }),
      tilingSprite && ((tilingSprite.width = P.x), (tilingSprite.height = P.y)),
      (sceneContainer.hitArea = new PIXI.Rectangle(0, 0, P.x, P.y));
  }
  ...
  new Map(),
    (window.onload = function () {
      (gameStateManager = ne.getInstance()),
        (particleManager = new Qe()),
        (graveyardManager = new Oe()),
        (golemManager = new Ue()),
        (skeletonChampionManager = new Xe()),
        (zombieManager = new Ae()),
        (humanManager = new Se()),
        gameStateManager.loadData(),
        gameStateManager.onReady(),
        O(),
        (function () {
          ...
          const e = new PIXI.Application({
            ...
            resolution: gameStateManager.persistentData.resolution || 1,
            antialias: !1,
            resizeTo: window,
          });
          document.body.appendChild(e.view),
            ...
            (function (e) {
              (sceneContainer = new PIXI.Container()),
                ...
                (unitContainer.sortableChildren = !0),
                ...
                (renderTextureSprite.visible = !1),
                (renderTextureSprite.alpha = 0),
                spellContainer.addChild(renderTextureSprite),
                sceneContainer.addChild(groundContainer),
                sceneContainer.addChild(effectContainer),
                sceneContainer.addChild(unitContainer),
                sceneContainer.addChild(uiContainer),
                e.stage.addChild(sceneContainer),
                e.stage.addChild(spellContainer),
                (sceneContainer.interactive = !0),
                (sceneContainer.interactiveChildren = !1),
                sceneContainer.on("pointerdown", startDragging),
                sceneContainer.on("pointerup", stopDragging),
                sceneContainer.on("pointerupoutside", stopDragging),
                sceneContainer.on("pointermove", handleDragging),
                sceneContainer.on("click", handleGroundClick),
                sceneContainer.on("tap", handleGroundClick),
                (document.getElementsByTagName("canvas")[0].onwheel =
                  handleScrollWheel),
            ...
            e.loader
              ...
              .load(function () {
                (gameStateManager.app = e),
                  updateMap(),
                  (tilingSprite = new PIXI.TilingSprite(
                    PIXI.Texture.from("grass.png"),
                  )),
                  (tilingSprite.texture.baseTexture.mipmap =
                    PIXI.MIPMAP_MODES.OFF),
                  (tilingSprite.width = P.x),
                  (tilingSprite.height = P.y),
                  groundContainer.addChild(tilingSprite),
                  gameStateManager.setupLevel(),
                  setTimeout(function () {
                    Z(!0);
                  }),
                  e.ticker.add((t) => {
                    gameLoop(e.ticker.deltaMS / 1e3, e),
                      (gameStateManager.frameRate = e.ticker.FPS);
                  });
              });
        })(),
            ...
            : (-1 === document.referrer.indexOf("kongregate.com") &&
              ...
              kongregateAPI.loadAPI(function () {
                (window.kongregate = kongregateAPI.getAPI()),
                  (gameStateManager.kongregate = !0),
                  gameStateManager.loginInUsingPlayFab();
              })),
        document.addEventListener(
          "visibilitychange",
          function () {
            "hidden" == document.visibilityState
              ? (gameStateManager.hidden = !0)
              : (gameStateManager.hidden = !1);
          },
          !1,
        );
    }),
  ...
  class ee {
    ...
    getRandomBuilding() {
      return randomItem(this.buildingsByPopularity, Math.random());
    }
    roomNoOverlap(e, t) {
      return (
        e.x > t.x + t.width + 50 ||
    ...
    addBuilding(e) {
      ...
      (e.entrance = n),
        ...
        (e.walls = []);
      const l = randomItem(this.buildingTextures, Math.random());
      ...
      (e.container.cacheAsBitmap = !0), groundContainer.addChild(e.container);
    ...
    populateTrees() {
      ...
      for (; e > 0; ) {
        ...
        if (i) {
          ...
          this.gameModel.isBossStage(this.gameModel.level) &&
            Math.random() > 0.7 &&
            (r = randomItem(this.armyTextures, Math.random())),
  ...
  class se {
    ...
    upgradeMaxPrice(e, t) {
      return totalPrice(e.basePrice, e.multi, this.currentRank(e), t);
    }
  ...
  class ne {
    ...
    update(e, t) {
      ...
      this.spells.updateSpells(e),
        (e *= this.gameSpeed),
        this.hidden && gameLoop(e, this.app),
  ...
  class oe {
    ...
    displayStatValue(e) {
      switch (e.type) {
        case this.types.energyRate:
          return (
            "Energy rate: " +
            buildingDistance(this.gameModel.energyRate) +
            " per second"
          );
        case this.types.energyCap:
          return "Maximum energy: " + unitHealth(this.gameModel.energyMax);
        case this.types.bloodCap:
          return "Maximum blood: " + unitHealth(this.gameModel.bloodMax);
        case this.types.brainsCap:
          return "Maximum brains: " + unitHealth(this.gameModel.brainsMax);
        case this.types.damage:
          return "Zombie damage: " + unitHealth(this.gameModel.zombieDamage);
        case this.types.speed:
          return "Zombie speed: " + unitHealth(this.gameModel.zombieSpeed);
        case this.types.health:
          return (
            "Zombie maximum health: " + unitHealth(this.gameModel.zombieHealth)
          );
        case this.types.brainRecoverChance:
          return (
        ...
        case this.types.boneCollectorCapacity:
          return (
            ...
            unitHealth(this.gameModel.boneCollectorCapacity)
          );
        case this.types.bonesGainPC:
          return (
            "Bones: " +
            unitHealth(Math.round(100 * this.gameModel.bonesPCMod)) +
            "%"
          );
        case this.types.partsGainPC:
          return (
            "Parts: " +
            unitHealth(Math.round(100 * this.gameModel.partsPCMod)) +
            "%"
          );
        case this.types.bloodGainPC:
          return (
            "Blood: " +
            unitHealth(Math.round(100 * this.gameModel.bloodPCMod)) +
            "%"
          );
        case this.types.bloodStoragePC:
          return (
            "Blood Storage: " +
            unitHealth(100 * this.gameModel.bloodStorePCMod) +
            "%"
          );
        case this.types.brainsGainPC:
          return (
            "Brains: " +
            unitHealth(Math.round(100 * this.gameModel.brainsPCMod)) +
            "%"
          );
        case this.types.brainsStoragePC:
          return (
            "Brains Storage: " +
            unitHealth(100 * this.gameModel.brainsStorePCMod) +
            "%"
          );
        case this.types.zombieDmgPC:
          return (
        ...
        case this.types.AvionicsPC:
          return "Harpy speed: " + unitHealth(this.gameModel.harpySpeed);
        case this.types.SkeleMove:
          return "Skeleton speed: " + unitHealth(this.gameModel.SkeleMoveMod);
        case this.types.ShockPC:
          return (
        ...
        case this.types.harpySpeed:
          return "Harpy speed: " + unitHealth(this.gameModel.harpySpeed);
        case this.types.harpyBombs:
          return "Harpy bombs: " + unitHealth(this.gameModel.harpyBombs);
        case this.types.tankBuster:
          return this.currentRank(e) > 0
    ...
    upgradeMaxPrice(e, t) {
      return totalPrice(e.basePrice, e.multiplier, this.currentRank(e), t);
    }
    canAffordUpgrade(e) {
    ...
    shatterBloodCost(e) {
      return Math.max(
        0,
        1e8 * Math.pow(1.5, this.gameModel.persistentData.runeshatter) -
          e.blood,
      );
    }
    shatterEffect() {
  ...
  class Se {
    ...
    setupVipText(e) {
      this.vipText ||
        ...
        uiContainer.addChild(this.vipText)),
    ...
    populate() {
      ...
      if (this.humans.length > 0) {
        for (let e = 0; e < this.humans.length; e++)
          unitContainer.removeChild(this.humans[e]), this.humans[e].stop();
      ...
      if (e.visible) {
        if (
          ...
          return (
            ...
            (e.visible = !1),
            void unitContainer.removeChild(e)
          );
        (e.alpha -= this.fadeSpeed * t),
          e.alpha < 0 && ((e.visible = !1), unitContainer.removeChild(e));
      }
    }
    changeState(e, t) {
      switch (t) {
    ...
    populate() {
      ...
      if (this.police.length > 0) {
        for (let e = 0; e < this.police.length; e++)
          unitContainer.removeChild(this.police[e]);
    ...
    populate() {
      ...
      if (
        (this.droneStrike &&
          ...
          (uiContainer.removeChild(this.droneStrike.text),
          uiContainer.removeChild(this.droneStrike.laser)),
        this.armymen.length > 0)
      ) {
        for (let e = 0; e < this.armymen.length; e++)
          unitContainer.removeChild(this.armymen[e]);
        ...
        if (
          ...
          this.droneStrike.startedBombing ||
            (this.droneStrike.text ||
              ...
              uiContainer.addChild(this.droneStrike.text),
              (this.droneStrike.laser = new PIXI.Graphics()),
              uiContainer.addChild(this.droneStrike.laser)),
    ...
    populate() {
      ...
      if (this.tanks.length > 0) {
        for (let e = 0; e < this.tanks.length; e++)
          unitContainer.removeChild(this.tanks[e]);
  ...
  function He(e, t, s) {
    if (ne.getInstance().persistentData.particles)
      if (Re.length > 0) {
        const i = Re.pop();
        i.reset(), (i.text = buildingDistance(s)), i.position.set(e, t);
      } else {
        const i = new ze(buildingDistance(s), Ie);
        uiContainer.addChild(i),
          i.position.set(e, t),
          i.anchor.set(0.5, 1),
          i.scale.set(0.2, 0.2),
          Be.push(i);
  ...
  class Ae {
    ...
    populate() {
      ...
      if (this.zombies.length > 0) {
        for (let e = 0; e < this.zombies.length; e++)
          unitContainer.removeChild(this.zombies[e]), this.zombies[e].stop();
      ...
      if (!this.zombieCursor) {
        ...
        (e.alpha = 0.6),
          ...
          spellContainer.addChild(this.zombieCursor);
      }
    }
    ...
    causePlagueExplosion(e, t, s = !0, i = !1) {
      ...
      this.blood.newPlagueSplatter(e.x, e.y),
        ...
        s && ((e.visible = !1), unitContainer.removeChild(e));
      for (let s = 0; s < this.aliveHumans.length; s++)
    ...
    updateZombie(e, t) {
      ...
      if (e.flags.dead) {
        ...
        return (
          (e.alpha -= this.fadeSpeed * t),
          void (e.alpha < 0 && ((e.visible = !1), unitContainer.removeChild(e)))
        );
      }
      switch (
        (1 == e.mod &&
    ...
    assignRandomTarget(e) {
      ...
      e.target = randomItem(this.aliveHumans, Math.random());
    }
    dotProduct(e, t) {
      return e * e + t * t;
    }
  ...
  class Xe {
      ...
      for (let t = 0; t < this.skeletons.length; t++)
        this.skeletons[t].flags.dead
          ? (this.discardedSprites.push(this.skeletons[t]),
            unitContainer.removeChild(this.skeletons[t]))
        ...
        for (let s = 0; s < e.e.length; s++)
          switch (e.e[s]) {
            ...
            case this.stats.zombieHealth.id:
              t.push(
                "+" +
                  unitHealth(this.stats.zombieHealth.scaling * e.l) +
                  " zombie health",
              );
              break;
            case this.stats.zombieDamage.id:
              t.push(
                "+" +
                  unitHealth(this.stats.zombieDamage.scaling * e.l) +
                  " zombie damage",
              );
              break;
            case this.stats.zombieSpeed.id:
    ...
    generateLoot(e) {
      ...
      if (
        ...
        const e = randomItem(this.spells.spells, Math.random());
        i.push(e.id);
      }
        ...
        this.model.constructions.monsterFactory
          ? this.creatures[t].flags.dead
            ? (this.discardedSprites.push(this.creatures[t]),
              unitContainer.removeChild(this.creatures[t]))
  ...
  class Oe {
    ...
    drawGraveyard() {
      this.spikeTexture ||
        ...
        this.sprite && groundContainer.removeChild(this.sprite),
        this.fortSprite &&
          (unitContainer.removeChild(this.fortSprite),
      ...
      (this.sprite.width = 32),
        ...
        groundContainer.addChild(this.sprite),
        (this.sprite.x = s.x),
        (this.sprite.y = s.y),
        ...
        t &&
          ...
          unitContainer.addChild(this.fortSprite));
    }
    drawFence() {
      if (
        (this.fence ||
          ((this.fence = new PIXI.Container()),
          groundContainer.addChild(this.fence)),
      ...
      for (let r = 0; r < e; r++) {
        ...
        this.fencePosts[r]
          ...
          : ((e = new PIXI.Sprite(
              randomItem(this.fenceTextures, Math.random()),
            )),
  ...
  class Ve {
    ...
    addAndRemoveBoneCollectors() {
      if (this.sprites.length > this.gameModel.persistentData.boneCollectors) {
        ...
        this.gameModel.addBones(e.bones), unitContainer.removeChild(e);
      }
      if (this.sprites.length < this.gameModel.persistentData.boneCollectors) {
        ...
        case qe.bombing:
          if (!e.target || e.target.graveyard || e.target.dead)
            if (
              ...
              (e.target = randomItem(this.tanks.aliveTanks, Math.random())),
                (e.bomb.fire = !0);
            else {
              for (
                let t = 0;
    ...
    update(e) {
      ...
      if (null != this.targetElement) {
        ...
        (t = {
          ...
          (t.x -= sceneContainer.x),
          (t.y -= sceneContainer.y),
          (t.x = t.x / sceneContainer.scale.x),
          (t.y = t.y / sceneContainer.scale.y);
      }
    ...
    initialize() {
      if (
        ...
        this.container ||
          ...
          effectContainer.addChild(this.container),
  ...
  class rt {
    ...
    initialize() {
      ...
      for (let e = 0; e < this.sprites.length; e++)
        unitContainer.removeChild(this.sprites[e]);
  ...
  const St = [
      ...
      new xt(
        ...
        function () {
          const e = dt.talents[this.id];
          return e && e > 0
            ? `Skeleton killing blows reward ${10 * e}x of your current parts per second`
      ...
      .controller("ZombieController", [
        ...
        function (e, t, s) {
          ...
          (c.model = ne.getInstance()),
            ...
            (c.factoryTab = "parts"),
            (c.factoryStats = {}),
            (c.moveTooltip = adjustTooltipPosition),
            (c.confirmMessage = ""),
            ...
            (c.factory = {
              ...
              creaturePercent(e) {
                return Math.min(
                  Math.round(
                    (c.model.persistentData.parts / this.creaturePrice(e)) *
                      100,
                  ),
                  100,
                );
              ...
              creatureButtonText(e) {
                return e.building
                  ...
                  : this.creatureTooExpensive(e)
                    ? unitHealth(
                        this.creaturePrice(e) - c.model.persistentData.parts,
                      ) + " parts required"
                    : "Build (" + unitHealth(this.creaturePrice(e)) + " parts)";
              },
              creatureLevelButtonText(e) {
                return this.canLevelCreature(e)
                  ? "Upgrade Level " +
                      (e.level + 1) +
                      " (" +
                      unitHealth(this.creatureLevelPrice(e)) +
                      " parts)"
                  : unitHealth(
            ...
            (c.upgradeSubtitle = function (e) {
              switch (e.type) {
                ...
                case h.types.bloodCap:
                  return "+" + unitHealth(e.effect) + " max blood";
                ...
                case h.types.zombieDmgPC:
                  return (
                    "+" +
                    unitHealth(Math.round(100 * e.effect)) +
                    "% zombie damage"
                  );
                ...
                case h.types.plagueDamage:
                  return "+" + unitHealth(e.effect) + " plague damage";
                case h.types.plagueTicks:
                  return "+" + unitHealth(e.effect) + " plague ticks";
            ...
            (c.requiredForUpgrade = function (e) {
              ...
              switch (e.costType) {
                case h.costs.energy:
                  return unitHealth(t - c.model.energy) + " energy required";
                case h.costs.blood:
                case r.costs.blood:
                  return (
                    unitHealth(t - c.model.persistentData.blood) +
                    " blood required"
                  );
                case h.costs.brains:
                  return (
                    unitHealth(t - c.model.persistentData.brains) +
                    " brains required"
                  );
                case h.costs.bones:
                  return (
                    unitHealth(t - c.model.persistentData.bones) +
                    " bones required"
                  );
                case h.costs.prestigePoints:
                  return (
                    unitHealth(
                      t - c.model.persistentData.prestigePointsToSpend,
                    ) + " prestige points required"
                  );
                case r.costs.parts:
                  return (
                    unitHealth(t - c.model.persistentData.parts) +
                    " parts required"
                  );
              }
            }),
              ...
              if (c.keysPressed.shift) {
                if (c.sidePanels.factory) {
                  ...
                  return (
                    ...
                    t +
                    " (" +
                    unitHealth(r.upgradeMaxPrice(e, t)) +
                    " " +
                ...
                }
                {
                  ...
                  return (
                    ...
                    t +
                    " (" +
                    unitHealth(h.upgradeMaxPrice(e, t)) +
                    " " +
                    c.costTranslate(e.costType) +
                    ")"
                  );
            ...
            (c.infuseButtonText = function () {
              return c.infusionMax ? "Max" : unitHealth(c.infusionAmount);
            }),
